...
#endif
static int do_updates();
static void event_inputchange();
...
void find_event() {
    ...
    for (scan = event_list, flag = 0; scan; scan = scan->next) {
        if (((scan->window == ANY) || (context == scan->window)) &&
            ((scan->device == ANY) || (device == scan->device)) && ((scan->state == ANY) || (state == scan->state))) {
...
void delete_events(int context, int device, int state, void *func) {
    event_p scan, prev;

    prev = 0;
    scan = event_list;
    while (scan) {
        if (((context == ANY) || (context == scan->window)) && ((device == ANY) || (device == scan->device)) &&
            ((state == ANY) || (state == scan->state)) && ((func == (void *)ANY) || (func == scan->func))) {
            event_p nuke;
...
void delete_updates(int *flag, void *func) {
    update_p scan, prev;

    prev = 0;
    scan = update_list;
    while (scan) {
        if (((flag == scan->flag) || (flag == (int *)ANY)) && ((func == scan->ufunc) || (func == (void *)ANY))) {
            update_p nuke;
...
void setInt(void *i) { *(int *)i = 1; }

void resetInt(void *i) { *(int *)i = 0; }

void setIntToVal(void *i, int v) { *(int *)i = v; }
