...
int main (int argc, char	*argv[]) {
	...
	*/
...

	{
		...
		glLoadIdentity(); 
		gluPerspective(45.0, (float)XMAXSCREEN / YMAXSCREEN, THICK, 9.0);
		glMatrixMode(mm); 
	}
...
char *str;
{
	...
	if (chrtbl[*str][0][0]) {
		...
		switch (j) {
		    ...
		    case 3: 
				glVertex2i(chrtbl[*str][i][1], chrtbl[*str][i][2]);
			break;
...
button_struct *button;
{
	...
	glLineWidth((sizex*3/1000 + 1) * 800 / 1280);

    switch (button->wc) {

	case 1:
		glBegin(GL_LINE_STRIP); 
			glVertex2i(-6 * (long)strlen(button->name[0]) / 2, -4);
	...
	case 2:
	    glBegin(GL_LINE_STRIP); 
			glVertex2i(-6 * (long)strlen(button->name[0])/2, 1);
			stroke(button->name[0]);
			glVertex2i(-6 * (long)strlen(button->name[1])/2, -9);
	...
	case 3:
		glBegin(GL_LINE_STRIP);
			glVertex2i(-6 * (long)strlen(button->name[0])/2, 6);
			stroke(button->name[0]);
			glVertex2i(-6 * (long)strlen(button->name[1])/2, -4);
			stroke(button->name[1]);
			glVertex2i(-6 * (long)strlen(button->name[2])/2, -14);
