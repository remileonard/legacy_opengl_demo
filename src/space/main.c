...
static char *space_assist[] = {
         " COMMAND LINE ARGS",
         "   -h       : help",
         "   -f path  : set data directory path",
         "",
         0
} ;

static char *space_help[] = {
         " MOUSE CONTROLS",
         "    left   : accelerates",
         "    middle : brakes",
...
static char *opening_credit[] = {
         "SGI Space Simulator Version 2.00",
         "",
         "",
         "A simulation of the Solar System and the Milky Way galaxy",
         "with certain artistic liberties.",
         0
} ;

static char *geosphere_credit[] = {
...
static char *sub_geosphere_credit[] = {
         "SATELLITE EARTH COLOR DATA",
         "Tom Van Sant/The Geosphere Project",
         "L.V. Warren, L. Blume, J. Knighton",
         "Santa Monica, CA  310-459-4342",
         0
} ;
...
{  char   *token ;
   ...
   while (--argc > 0 && **++argv == '-')
       for (token = argv[0]+1; *token; token++)
          switch (*token) {
            case 'd': Counter.flags &= ~FLSCR_FLAG ;
                      break ;
            case 'f': Counter.flags |= DPATH_FLAG ;
                      argc-- ;
                      strcpy(datapath,*++argv) ;
                      strcat(datapath,"/") ;
                      break ;
            case 'h': for (i=0; space_assist[i] != NULL; i++)
                        fprintf(stdout,"\t%s\n", space_assist[i]);
                      for (i=0; space_help[i] != NULL; i++)
                        fprintf(stdout,"\t%s\n", space_help[i]);
                      exit(0) ;
                      break ;
            case 's': Counter.flags |= SPACB_FLAG ;
                      break ;
   ...
   if (!spInitFont("-adobe-times-bold-r-normal--0-150-100-100-p-100-iso8859-1")) {
     ...
     printf("Cannot initialize font, please enter name of new font:");
     scanf("%s",cha);

     if (!spInitFont(cha)) {
       printf("Cannot find alternate font");
       exit(-1);
       }
     }
