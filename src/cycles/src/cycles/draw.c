...
printf("\n");
	...
	i = C->trail_ptr;
	v[1] = 0.0;
	if (C->trail[i].level == draw_level && MAX(ABS(C->trail[i].x - C->origin.x), ABS(C->trail[i].z - C->origin.z)) > CYCLE_TAIL) {
	    bgnpolygon();
	    v[0] = C->trail[i].x;
	    v[2] = C->trail[i].z;
	    v3f(v);
