...
#endif
    bgntmesh();
    v[0] = -0.9; v[1] = -0.1; v2f(v);
    v[0] = -0.7; v[1] =  0.4; v2f(v);
    v[0] =  0.0; v[1] = -0.7; v2f(v);
    v[0] =  0.0; v[1] =  0.0; v2f(v);
    v[0] =  1.0; v[1] =  0.9; v2f(v);
    v[0] =  0.8; v[1] =  1.0; v2f(v);
    v[0] =  1.4; v[1] =  1.3; v2f(v);
    endtmesh();    
}
...
#endif
    bgntmesh();
    v[0] = 0.1; v[1] = 0.4; v2f(v);
    v[0] = 0.3; v[1] = 0.3; v2f(v);
    v[0] = 0.3; v[1] = 0.5; v2f(v);
    endtmesh();
#ifdef RGB_MODE
    cpack(0);
#else
...
#endif
    bgntmesh();
    v[0] = 0.9; v[1] = 0.4; v2f(v);
    v[0] = 0.7; v[1] = 0.3; v2f(v);
    v[0] = 0.7; v[1] = 0.5; v2f(v);
    endtmesh();
#ifdef RGB_MODE
    cpack(0);
#else
...
int over_area(void) {
	...
	dx = but[i][0] - x;
	dy = but[i][1] - y;
	if (dx*dx + dy*dy < 0.08*0.08) return (i);
    }

    /* left arrow */
    if (x < 0.3 && y < 0.35+0.5*x && y > 0.45-0.5*x) return(10);
    /* right arrow */
    if (x > 0.7 && y < 0.85-0.5*x && y > -0.05+0.5*x) return(11);
...
void title_screen(int screen_num, char *name, int *colour_choice, int *num_robots) {
    ...
    unqdevice(RETKEY);
    if (screen_num == 3) unqdevice(KEYBD);
