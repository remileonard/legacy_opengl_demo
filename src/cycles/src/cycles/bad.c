...
void move_cycles(CYCLE *C) {
    ...
    if (front < margin) {
        ...
        wall_prox = DIM - ABS(vec[C->vec_ptr][0] * C->origin.x) - ABS(vec[C->vec_ptr][1] * C->origin.z);
        ...
        if ((left > margin) || (right > margin))
            if (jump_dist < front)
                ...
                if (behave) {
                    if (((left > margin) && ((vec[l][0] == xdes) || (vec[l][1] == zdes))) || (right <= margin))
                        turn(C, 1.0);
                    if (((right > margin) && ((vec[r][0] == xdes) || (vec[r][1] == zdes))) || (left <= margin))
                        turn(C, -1.0);
                } else if (left < right)
                    turn(C, -1.0);
                else
        ...
        if ((left < margin) && (right < margin) && !C->jump)
            jump(C);
        if (C->step > MIN_STEP)
            C->step -= 0.3 * speed_fac;
    } else {
        ...
        if (!behave) {
            ...
            if (random_divisor < 1)
                random_divisor = 1; // Éviter division par zéro
        ...
        if ((front > 0.66 * DIM) && (C->step < MAX_STEP))
            C->step += 0.1 * speed_fac;
        if ((front < 0.25 * DIM) && (C->step > MIN_STEP))
            C->step -= 0.2 * speed_fac;
    }
...
CYCLE *find_nearest_person(CYCLE *C) {
    ...
    for (i = 0; i < CYCLES; i++) {
        if (used[i] && bike[i].alive && bike[i].type == PERSON && bike[i].falling == 0 && bike[i].level == C->level) {
