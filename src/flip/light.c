...
static float ldef_alpha[] = {ALPHA, 0.2, LMNULL};
...
void defineshading(void) {
    ...
    for (i = 0; i < NUM_MATERIALS; i++) {
        ...
        if (alpha) {
            lmdef(DEFMATERIAL, i + 1 + NUM_MATERIALS, SIZE(ldef_material[i]), ldef_material[i]);
            lmdef(DEFMATERIAL, i + 1 + NUM_MATERIALS, SIZE(ldef_alpha), ldef_alpha);
        }
    }
...
void resetmodel() { lmbind(LMODEL, curmodel + 1); }

static float lmat[] = {EMISSION, 0.0, 0.0, 0.0, DIFFUSE, 0.0, 0.0, 0.0, AMBIENT, 0.0, 0.0, 0.0, SHININESS, 0.0, LMNULL};
...
int draw_lights(void) {
    ...
    for (i = 0; i < NUM_LIGHTS; i++) {
        ...
        pushmatrix();
        translate(ldef_light[i][9], ldef_light[i][10], ldef_light[i][11]);
...
/* VERTEX DATA */
static float vdata[] = {
    0.607062,  0,        -0.794654, -0.303531, 0.525731, -0.794655, -0.303531, -0.525731, -0.794655,
    0.491123,  0.850651, -0.187592, -0.982247, 0,        -0.187592, 0.491123,  -0.850651, -0.187592,
    -0.491123, 0.850651, 0.187592,  0.982247,  0,        0.187592,  -0.491123, -0.850651, 0.187592,
    0.303531,  0.525731, 0.794655,  -0.607062, 0,        0.794655,  0.303531,  -0.525731, 0.794655,
};
...
char *matnames[NUM_MATERIALS] = {
    "Brass",      "Shiny Brass", "Pewter",      "Silver",        "Gold",
    "Shiny Gold", "Plaster",     "Red Plastic", "Green Plastic", "Blue Plastic",
};

float ldef_material[][15] = {
    {/* BRASS */
     AMBIENT, .35, .25, .1, DIFFUSE, .65, .5, .35, SPECULAR, .8, .6, 0., SHININESS, 5., LMNULL},
    {/* SHINY BRASS */
     AMBIENT, .25, .15, 0., DIFFUSE, .65, .5, .35, SPECULAR, .9, .6, 0., SHININESS, 10., LMNULL},
    {/* PEWTER */
     AMBIENT, .1, .1, .1, DIFFUSE, .6, .55, .65, SPECULAR, .9, .9, .95, SHININESS, 10., LMNULL},
    {/* SILVER */
     AMBIENT, .4, .4, .4, DIFFUSE, .3, .3, .3, SPECULAR, .9, .9, .95, SHININESS, 30., LMNULL},
    {/* GOLD */
     AMBIENT, .4, .2, 0., DIFFUSE, .9, .5, 0., SPECULAR, .7, .7, 0., SHININESS, 10., LMNULL},
    {/* SHINY GOLD */
     AMBIENT, .4, .2, 0., DIFFUSE, .9, .5, 0., SPECULAR, .9, .9, 0., SHININESS, 2., LMNULL},
    {/* PLASTER */
     AMBIENT, .2, .2, .2, DIFFUSE, .95, .95, .95, SPECULAR, 0., 0., 0., SHININESS, 1., LMNULL},
    {/* RED PLASTIC */
     AMBIENT, .3, .1, .1, DIFFUSE, .5, .1, .1, SPECULAR, .45, .45, .45, SHININESS, 30., LMNULL},
    {/* GREEN PLASTIC */
     AMBIENT, .1, .3, .1, DIFFUSE, .1, .5, .1, SPECULAR, .45, .45, .45, SHININESS, 30., LMNULL},
    {/* BLUE PLASTIC */
     AMBIENT, .1, .1, .3, DIFFUSE, .1, .1, .5, SPECULAR, .45, .45, .45, SHININESS, 30., LMNULL}};

char *lightnames[NUM_LIGHTS] = {"White Infinite", "Red Local",        "Blue Infinite", "Green Local",
                                "Yellow Local",   "Magenta Infinite", "White Local",   "Pink Infinite"};
...
float ldef_light[][14] = {
    ...
    {/* BLUE INFINITE */
     LCOLOR, .1, .1, .9, AMBIENT, 0., 0., .05, POSITION, 0.2, -0.2, 0.2, 0., LMNULL},
    {/* GREEN LOCAL */
     LCOLOR, .04, .9, .04, AMBIENT, .0, 0., 0., POSITION, -0.2, -0.2, 0.2, 1., LMNULL},
    {/* YELLOW LOCAL */
     LCOLOR, .8, .8, .3, AMBIENT, .05, .05, 0., POSITION, -0.2, 0.2, -0.2, 1., LMNULL},
    {/* MAGENTA INFINITE */
     LCOLOR, .8, 0., .7, AMBIENT, .0, .0, .0, POSITION, 0.2, 0.2, -0.2, 0., LMNULL},
    {/* WHITE LOCAL */
     LCOLOR, 1.0, 1.0, 1.0, AMBIENT, 0., 0., 0., POSITION, 0.2, -0.2, -0.2, 1., LMNULL},
    {/* PINK INFINITE */
     LCOLOR, .9, .5, .65, AMBIENT, 0., 0., 0., POSITION, -0.2, -0.2, -0.2, 0., LMNULL},
};
...
float ldef_lmodel[][10] = {
    {/* Infinite viewer */
     LOCALVIEWER, 0., AMBIENT, .3, .3, .3, ATTENUATION, 1., 0., LMNULL},
    {/* Local viewer */
     LOCALVIEWER, 1.0, AMBIENT, .3, .3, .3, ATTENUATION, 1.0, 0.02, LMNULL},
};
