...
void SetCamera(void)
{
    ...
    // Utilisation du temps écoulé pour des angles indépendants du taux d'image
    float t = glutGet(GLUT_ELAPSED_TIME) * 0.001f; // secondes
    float camOsc1 = sinf(t * 0.8f) * 0.5f; // oscillation douce
    float camOsc2 = cos(t)*0.5f;
    ...
    glTranslated(0.0, 0.4 + camOsc1, 0.0);
    glRotated(angle * 5.0 + camOsc2 * 10.0, 0.0, 1.0, 0.0);
    glTranslatef(0.0, 0.0, -6.5 - 2 * camOsc1 - camOsc2);
    glRotatef(45 - 35 * camOsc2, 1.0, 0.0, 0.0);
    glRotatef(-100 * camOsc2, 0.0, 1.0, 0.0);
...
void Animate(void)
{
    ...
    delta_time = (current_time - last_time) / 1000.0f; // secondes
    last_time = current_time;
    ...
    speed = 2.0;
    plaatje += speed * delta_time * 30.0; // ajuster la vitesse
